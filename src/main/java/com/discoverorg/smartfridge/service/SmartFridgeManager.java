...
public interface SmartFridgeManager {
  ...
  /**
   * Event Handlers - These are methods invoked by the SmartFridge hardware to
   * send notification of items that have been added and/or removed from the
   * fridge. Every time an item is removed by the fridge user, it will emit a
   * handleItemRemoved() event to this class, every time a new item is added or a
   * previously removed item is re-inserted, the fridge will emit a
   * handleItemAdded() event with its updated fillFactor.
   */
  ...
  /**
   * These are the query methods for the fridge to be able to display alerts and
   * create shopping lists for the fridge user.
   */
   ...
   *
   * i.e. getItems( 0.5 ) - will return any items that are 50% or less full,
   * including items that are depleted. Unless all available containers are empty,
   * this method should only consider the non-empty containers when calculating
   * the overall fillFactor for a given item.
   *
  ...
  /**
   * Stop tracking a given item. This method is used by the fridge to signal that
   * its owner will no longer stock this item and thus should not be returned from
   * #getItems()
   *
   * @param itemType
   */
