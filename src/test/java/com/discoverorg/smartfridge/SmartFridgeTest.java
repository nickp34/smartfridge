...
public class SmartFridgeTest {
  ...
  public void testFillMin() {
    ...
    smartFridge.handleItemAdded(i.getType(), i.getUUID(), i.getName(), i.getFillFactor());
    Double min = 0.0;
    assertEquals(min, smartFridge.getFillFactor(1));
  }

  @Test
  public void testFillMax() {
    Item i = new Item();
    i.setType(1);
    i.setFillFactor(2000.00);
    i.setUUID("b-1x");
    i.setName("Item 1");

    smartFridge.handleItemAdded(i.getType(), i.getUUID(), i.getName(), i.getFillFactor());
    Double max = 100.0;
    assertEquals(max, smartFridge.getFillFactor(1));
  }

  @Test
  public void testFillMaxMin() {
    Item i = new Item();
    i.setType(1);
    i.setFillFactor(2000.00);
    i.setUUID("b-1x");
    ...
    // Because 2000 will be changed to 100 and -60 will be ignored
    Double ave = 100.0;
    assertEquals(ave, smartFridge.getFillFactor(1));

    i.setFillFactor(300.00);
    i.setUUID("d-1x");

    // 300 will be changed to 100
    // (100 + 100) / 2
    smartFridge.handleItemAdded(i.getType(), i.getUUID(), i.getName(), i.getFillFactor());
    ave = 100.0;
    assertEquals(ave, smartFridge.getFillFactor(1));
